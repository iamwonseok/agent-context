#!/usr/bin/env python3
"""
Fail if Markdown files contain emoji or decorative symbols.

This script is intended to be used as a pre-commit hook to enforce the
repository policy:
- Markdown may be Korean/English
- Emoji and decorative Unicode symbols are forbidden
"""
...
def scan_file(path: Path, max_findings: int = 20) -> list[Finding]:
    try:
        ...
        # Markdown should be UTF-8. If not, treat as failure with a clear message.
        return [
            Finding(
                path=str(path),
                line=1,
...
def main(argv: list[str]) -> int:
    ...
    print("[NG] Markdown contains forbidden emoji/symbols:", file=sys.stderr)
